...
config.plugins.esipanel = ConfigSubsection()
config.plugins.esipanel.swapautostart = ConfigYesNo(default = False)
...
def SwapAutostart(reason, session=None, **kwargs):
	global startswap
	if reason == 0:
		if config.plugins.esipanel.swapautostart.value:
...
class Swap(Screen):
	...
	def getSwapDevice(self):
		...
		if path.exists('/etc/rcS.d/S98SwapManager'):
			...
			config.plugins.esipanel.swapautostart.value = True
			config.plugins.esipanel.swapautostart.save()
	...
	def updateSwap2(self, result = None, retval = None, extra_args = None):
		...
		if config.plugins.esipanel.swapautostart.value and self.swap_place:
			self['autostart_off'].hide()
			self['autostart_on'].show()
		else:
			config.plugins.esipanel.swapautostart.value = False
			config.plugins.esipanel.swapautostart.save()
			configfile.save()
			self['autostart_on'].hide()
			self['autostart_off'].show()
