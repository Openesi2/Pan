...
config.plugins.esipanel_redpanel = ConfigSubsection()
...
if Check_Softcam():
	redSelection = [('0',_("Default (Instant Record)")), ('1',_("EsiPanel")),('2',_("Timer List")),('3',_("Show Movies")), ('4',_("Softcam Panel"))]
else:
	redSelection = [('0',_("Default (Instant Record)")), ('1',_("EsiPanel")),('2',_("Timer List")),('3',_("Show Movies"))]
...
def timerEvent():
	...
	if getBoxType() == "dm800":
		config.plugins.esipanel_redpanel.selection = ConfigSelection(redSelection, default='0')
		config.plugins.esipanel_redpanel.selectionLong = ConfigSelection(redSelection, default='1')
	else:
		config.plugins.esipanel_redpanel.selection = ConfigSelection(redSelection, default='1')
		config.plugins.esipanel_redpanel.selectionLong = ConfigSelection(redSelection, default='2')
timer = eTimer()
...
config.plugins.esipanel_yellowkey = ConfigSubsection()
if getBoxType() == "dm800":
	config.plugins.esipanel_yellowkey.list = ConfigSelection(default='1', choices = choicelist)
	config.plugins.esipanel_yellowkey.listLong = ConfigSelection(default='1', choices = choicelist)
else:
	config.plugins.esipanel_yellowkey.list = ConfigSelection(default='0', choices = choicelist)
	config.plugins.esipanel_yellowkey.listLong = ConfigSelection(default='0', choices = choicelist)
config.plugins.showesipanelextensions = ConfigYesNo(default=False)
config.plugins.esipanel_frozencheck = ConfigSubsection()
config.plugins.esipanel_frozencheck.list = ConfigSelection([('0',_("Off")),('1',_("1 min.")), ('5',_("5 min.")),('10',_("10 min.")),('15',_("15 min.")),('30',_("30 min."))])
...
from Plugins.Extensions.EsiPanel.SoftcamPanel import *
from Plugins.Extensions.EsiPanel.CamStart import *
from Plugins.Extensions.EsiPanel.CamCheck import *
...
if not Check_Softcam() and config.plugins.showesipanelextensions.value:
	config.plugins.showesipanelextensions.value = False
	config.plugins.showesipanelextensions.save()
	config.plugins.esipanel_redpanel.save()
...
def setDefaultKeymap():
	print "[Esi-Panel] Set Keymap to Default"
...
def main(session, **kwargs):
		session.open(EsiPanel)

def Apanel(menuid, **kwargs):
	if menuid == "mainmenu":
		return [("Esi Panel", main, "EsiPanel", 3)]
	else:
		return []

def camstart(reason, **kwargs):
	if not config.plugins.esipanel_frozencheck.list.value == '0':
		CamCheck()
	try:
		...
		print "[Esi-Panel] failed to write /proc/stb/video/alpha"
	...
	try:
		...
		print "[Esi-Panel] failed to run CamStart"
...
def InfoEntryComponent(file):
	...
	if png == None:
		png = LoadPixmap("/usr/lib/enigma2/python/Plugins/Extensions/EsiPanel/icons/" + file + ".png")
		if png == None:
			png = LoadPixmap(cached = True, path = resolveFilename(SCOPE_CURRENT_SKIN, "icons/default.png"));
			if png == None:
				png = LoadPixmap("/usr/lib/enigma2/python/Plugins/Extensions/EsiPanel/icons/default.png")
	res = (png)
	return res
...
class ShowSoftcamPanelExtensions(ConfigListScreen, Screen):
	...
	def createSetup(self):
		self.editListEntry = None
		self.list = []
		self.list.append(getConfigListEntry(_("Show Softcam-Panel in Extensions Menu"), config.plugins.showesipanelextensions))
		...
		self.list.append(getConfigListEntry(_("Frozen Cam Check"), config.plugins.esipanel_frozencheck.list))
