...
config.esipanel.cronmanager_commandtype = NoSave(ConfigSelection(choices = [ ('custom',_("Custom")),('predefined',_("Predefined")) ]))
...
config.esipanel.cronmanager_runwhen = NoSave(ConfigSelection(default='Daily', choices = [('Hourly', _("Hourly")),('Daily', _("Daily")),('Weekly', _("Weekly")),('Monthly', _("Monthly"))]))
config.esipanel.cronmanager_dayofweek = NoSave(ConfigSelection(default='Monday', choices = [('Monday', _("Monday")),('Tuesday', _("Tuesday")),('Wednesday', _("Wednesday")),('Thursday', _("Thursday")),('Friday', _("Friday")),('Saturday', _("Saturday")),('Sunday', _("Sunday"))]))
config.esipanel.cronmanager_dayofmonth = NoSave(ConfigInteger(default=1, limits=(1, 31)))
...
class SetupCronConf(Screen, ConfigListScreen):
	...
	def createSetup(self):
		...
		if f:
			...
			predefinedlist.sort()
		config.esipanel.cronmanager_predefined_command = NoSave(ConfigSelection(choices = predefinedlist))
		self.editListEntry = None
		self.list = []
		self.list.append(getConfigListEntry(_("Run how often ?"), config.esipanel.cronmanager_runwhen))
		if config.esipanel.cronmanager_runwhen.value != 'Hourly':
			self.list.append(getConfigListEntry(_("Time to execute command or script"), config.esipanel.cronmanager_cmdtime))
		if config.esipanel.cronmanager_runwhen.value == 'Weekly':
			self.list.append(getConfigListEntry(_("What Day of week ?"), config.esipanel.cronmanager_dayofweek))
		if config.esipanel.cronmanager_runwhen.value == 'Monthly':
			self.list.append(getConfigListEntry(_("What Day of month ?"), config.esipanel.cronmanager_dayofmonth))
		self.list.append(getConfigListEntry(_("Command type"), config.esipanel.cronmanager_commandtype))
		if config.esipanel.cronmanager_commandtype.value == 'custom':
			self.list.append(getConfigListEntry(_("Command To Run"), config.esipanel.cronmanager_user_command))
		else:
			self.list.append(getConfigListEntry(_("Command To Run"), config.esipanel.cronmanager_predefined_command))
	...
	def checkentry(self):
		msg = ''
		if (config.esipanel.cronmanager_commandtype.value == 'predefined' and config.esipanel.cronmanager_predefined_command.value == '') or config.esipanel.cronmanager_commandtype.value == 'custom' and config.esipanel.cronmanager_user_command.value == '':
	...
	def saveMycron(self):
		hour = '%02d' % config.esipanel.cronmanager_cmdtime.value[0]
		minutes = '%02d' % config.esipanel.cronmanager_cmdtime.value[1]
		if config.esipanel.cronmanager_commandtype.value == 'predefined' and config.esipanel.cronmanager_predefined_command.value != '':
			command = config.esipanel.cronmanager_predefined_command.value
		else:
			command = config.esipanel.cronmanager_user_command.value

		if config.esipanel.cronmanager_runwhen.value == 'Hourly':
			newcron = minutes + ' ' + ' * * * * ' + command.strip() + '\n'
		elif config.esipanel.cronmanager_runwhen.value == 'Daily':
			newcron = minutes + ' ' + hour + ' * * * ' + command.strip() + '\n'
		elif config.esipanel.cronmanager_runwhen.value == 'Weekly':
			if config.esipanel.cronmanager_dayofweek.value == 'Sunday':
				newcron = minutes + ' ' + hour + ' * * 0 ' + command.strip() + '\n'
			elif config.esipanel.cronmanager_dayofweek.value == 'Monday':
				newcron = minutes + ' ' + hour + ' * * 1 ' + command.strip() + '\n'
			elif config.esipanel.cronmanager_dayofweek.value == 'Tuesday':
				newcron = minutes + ' ' + hour + ' * * 2 ' + command.strip() + '\n'
			elif config.esipanel.cronmanager_dayofweek.value == 'Wednesday':
				newcron = minutes + ' ' + hour + ' * * 3 ' + command.strip() + '\n'
			elif config.esipanel.cronmanager_dayofweek.value == 'Thursday':
				newcron = minutes + ' ' + hour + ' * * 4 ' + command.strip() + '\n'
			elif config.esipanel.cronmanager_dayofweek.value == 'Friday':
				newcron = minutes + ' ' + hour + ' * * 5 ' + command.strip() + '\n'
			elif config.esipanel.cronmanager_dayofweek.value == 'Saturday':
				newcron = minutes + ' ' + hour + ' * * 6 ' + command.strip() + '\n'
		elif config.esipanel.cronmanager_runwhen.value == 'Monthly':
			newcron = minutes + ' ' + hour + ' ' + str(config.esipanel.cronmanager_dayofmonth.value) + ' * * ' + command.strip() + '\n'
		else:
			command = config.esipanel.cronmanager_user_command.value
		...
		out.close()
		rc = system('crontab /etc/cron/crontabs/root -c /etc/cron/crontabs')
		config.esipanel.cronmanager_predefined_command.value = 'None'
		config.esipanel.cronmanager_user_command.value = 'None'
		config.esipanel.cronmanager_runwhen.value = 'Daily'
		config.esipanel.cronmanager_dayofweek.value = 'Monday'
		config.esipanel.cronmanager_dayofmonth.value = "1"
		config.esipanel.cronmanager_cmdtime.value, mytmpt = ([0, 0], [0, 0])
		self.close()
